...
export const Navbar = () => {
  ...
  return <>
      ...
      y: 0
    }} className={`fixed top-0 lg:top-10 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-background/98 backdrop-blur-md shadow-lg lg:top-0" : "bg-background/95 backdrop-blur-sm"}`}>
        <nav className="container mx-auto px-6 py-3 flex justify-between items-center">
          {/* Logo */}
          <a href="#home" className="flex items-center gap-3">
            <div className="relative flex items-center gap-2">
              <div className="flex items-center">
                <span className="font-display text-xl md:text-2xl lg:text-3xl text-primary tracking-wider">ASTEHE</span>
          ...
          <div className="hidden lg:flex items-center gap-6">
            {navLinks.map(link => <a key={link.name} href={link.href} className="text-sm font-semibold text-foreground/80 hover:text-accent transition-colors uppercase tracking-wide flex items-center gap-1">
                {link.name}
        ...
        height: 0
            ...
            <div className="container mx-auto px-6 py-4 flex flex-col gap-4">
              {navLinks.map(link => <a key={link.name} href={link.href} onClick={() => setIsMobileMenuOpen(false)} className="text-foreground font-semibold py-2 hover:text-accent transition-colors uppercase tracking-wide">
                  {link.name}
                </a>)}
